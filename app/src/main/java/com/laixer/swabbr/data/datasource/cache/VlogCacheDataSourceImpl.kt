...
class VlogCacheDataSourceImpl constructor(
    ...
    override fun get(vlogId: UUID): Single<Vlog> = cache.load<List<Vlog>>(key).map { list -> list.first { it.id == vlogId } }
    ...
    override fun set(item: Vlog): Single<Vlog> =
        cache.load<List<Vlog>>(key).map { list -> list.filter { it.id != item.id }.plus(item) }.flatMap { set(it) }.map { item }
