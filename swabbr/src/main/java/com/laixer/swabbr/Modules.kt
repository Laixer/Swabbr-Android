...
val dataSourceModule: Module = module {
    single { AuthCacheDataSourceImpl(cache = get(named(AUTH_CACHE))) as AuthCacheDataSource }
    single { AuthRemoteDataSourceImpl(api = get(named(AUTH_API))) as AuthRemoteDataSource }
    single { UserCacheDataSourceImpl(cache = get(named(USER_CACHE))) as UserCacheDataSource }
    single { UserRemoteDataSourceImpl(api = get(named(USER_API))) as UserRemoteDataSource }
    single { VlogCacheDataSourceImpl(cache = get(named(VLOG_CACHE))) as VlogCacheDataSource }
    single { VlogRemoteDataSourceImpl(api = get(named(VLOG_API))) as VlogRemoteDataSource }
    single { ReactionCacheDataSourceImpl(cache = get(named(REACTION_CACHE))) as ReactionCacheDataSource }
    single { ReactionRemoteDataSourceImpl(api = get(named(REACTION_API))) as ReactionRemoteDataSource }
    single { FollowDataSourceImpl(api = get(named(FOLLOW_API))) as FollowDataSource }
    single { SettingsCacheDataSourceImpl(cache = get(named(SETTINGS_CACHE))) as SettingsCacheDataSource }
    single { SettingsRemoteDataSourceImpl(api = get(named(SETTINGS_API))) as SettingsRemoteDataSource }
}
val networkModule: Module = module {
    single(named(AUTH_INTERCEPTOR)) { AuthInterceptor(authCacheDataSource = get()) }
    single(named(RETROFIT)) { createNetworkClient(get(named(AUTH_INTERCEPTOR)), BASE_URL, BuildConfig.DEBUG) }
    single(named(AUTH_API)) { provideAuthApi(get(named(RETROFIT))) }
    single(named(USER_API)) { provideUsersApi(get(named(RETROFIT))) }
    single(named(VLOG_API)) { provideVlogsApi(get(named(RETROFIT))) }
    single(named(REACTION_API)) { provideReactionsApi(get(named(RETROFIT))) }
    single(named(FOLLOW_API)) { provideFollowApi(get(named(RETROFIT))) }
    single(named(SETTINGS_API)) { provideSettingsApi(get(named(RETROFIT))) }
}
val cacheModule: Module = module {
    single(named(AUTH_CACHE)) { ReactiveCache<Pair<String, String>>() }
...
private const val BASE_URL = "https://swabbr.azurewebsites.net/api/v1/"
