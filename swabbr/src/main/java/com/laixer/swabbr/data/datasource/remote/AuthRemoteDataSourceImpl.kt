...
class AuthRemoteDataSourceImpl constructor(
    ...
    override fun login(login: Login): Single<AuthUser> = api.login(login.mapToData()).map {
        AuthUser(
            it.accessToken, it.user.mapToDomain(), it.userSettings.mapToDomain()
        )
    }

    override fun register(registration: Registration): Single<AuthUser> = api.register(registration.mapToData()).map {
        AuthUser(
            it.accessToken, it.user.mapToDomain(), it.userSettings.mapToDomain()
        )
    }
